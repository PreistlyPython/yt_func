...
class yt_func():
    ...
    def get_top_videos(self, channel_id, query, max_results = 10, next_page_token = None): # Find top (max_results) videos based on views and using (next_page_token). Retuns list of video ids
        """
        ...
        while len(valid_videos) < max_results and search_attempt < 10:
            try:
                ...
                retry_attempt += 1
                if retry_attempt > 5:
                    self.logger.info(f"Saving {len(self.video_data.keys())} videos to file due to excessive retries.\nSaved to quota_exceeded_for_{self.subject_searched}.csv")
                    self.load_data_from_ids(valid_videos)
                    self.save_video_data_to_csv(f"quota_exceeded_for_{self.subject_searched}.csv")
    ...
    def search_youtube_videos(self, query, max_results=10, next_page_token=None, **kwargs):
        """
    ...
    def search_next_page_for_subjects(self): #Conduct search using next page information
        """
        ...
        for subject, tokens in subjects_and_tokens.items():
            for token in tokens:
                if token is not None and token.strip() != "":
                    ...
                    self.get_top_videos(subject, max_results=10, next_page_token=token)
    ...
    def get_transcripts(self, video_ids):
        ...
        for video_id in video_ids:
            if video_id in self.video_data:
                for video_data in self.video_data[video_id]:
                    try:
                        ...
                        video_data['transcript'] = "Transcript NOT Found"
            ...
            for video_data in video_data_list:
                ...
                if transcript and transcript != 'value not found':
                    ...
                    for prompt in self.script_prompts:
                        ...
                        script_reports.append("SCRIPT REPORT FOR | Video id: " + str(video_id) + ": " + script_report + "\n")
    ...
    def post_comment(video_id, text, credentials):
    ...
    def get_comments(self, video_id):
        """
        Retrieve comments for a video using YouTube API.
        """
        comments = []
    ...
    def add_comments(self, video_id, filename, seperate_save=False):
        """
        Retrieves comments for a video, saves them to a file, and updates self.video_data with top words.
        *to-do* Create a function that filters words for this function and other NLP functions
            to be able to get insights into topics and concerns, and feedback
        """
        # Check if video_id exists and has comments
        if video_id not in self.video_data or 'comment_count' not in self.video_data[video_id][0]:
            self.logger.error(f"No comment data available for video ID: {video_id}")
            return
        ...
        # Add top words to self.video_data
        if 'top_words' not in self.video_data[video_id][0]:
            self.video_data[video_id][0]['top_words'] = comments
        else:
            self.video_data[video_id][0]['top_words'].update(comments)
        ...
        if 'subject' in args:  # Check if 'subject' is passed in *args
            ...
            #Activate line above to iterate through subjects
                try:
                    ...
                    self.logger.info(f"Errror occured Adding {title} to {save_dir}... Calling save_graph")
            ...
            self.logger.info("Detected 'autosave in **kwargs'")
            try:
    ...
    def update_and_save(self, videos_to_find, output_filename, input_filename):
        ...
        self.search_next_page_for_subjects()
...
#yt.create_plots_for_directory("C:\Python\YTFunc\Charts\Virality", "Radar Charts Debugging", yt.plot_radar_chart, "radar_chart", autosave=True)

#yt.plot_view_count_distribution(pd.read_csv("virality_8_merged.csv"))

#yt.update_and_save(10, "virality_12.csv", "virality_11.csv")
yt.load_video_data_from_csv("virality_13_unique.csv")
vid_ids = list(yt.video_data.keys())
comments = [] 
for vid in vid_ids:
    print(f"Current video id = {vid}")
    comments.append(yt.get_comments(vid))
    yt.add_comments(vid, "virality_13_comments.csv")
yt.pp.pprint(comments)
